...
class Tool
{
    ...
    {
        // Verify that the source exists before copying
    ...
    {
        $timezone = is_object(\Auth::user()) && is_object(request()->selected_customer) ? request()->selected_customer->timezone : '';
        $result = $datetime;
        if (!empty($timezone)) {
            $result = $result->timezone($timezone);
        }
    ...
    public static function dayStringFromTimestamp($timestamp)
    {
        if (isset($timestamp) && $timestamp != '0000-00-00 00:00:00') {
            ...
            $result = Tool::dateTime(\Carbon\Carbon::now())->format('Y-m-d');
        }
    ...
    public static function timeStringFromTimestamp($timestamp)
    {
        if (isset($timestamp) && $timestamp != '0000-00-00 00:00:00') {
            ...
            $result = Tool::dateTime(\Carbon\Carbon::now())->format('H:i');
        }
    ...
    public static function cronjobUpdateController($request, $controller)
    {
        ...
        $setting_php_bin_path = Setting::get('php_bin_path');
        ...
        if (empty(Setting::get('remote_job_token'))) {
            Setting::set('remote_job_token', Str::random(60));
        }
        ...
        if ($request->isMethod('post')) {
            ...
            if ($request->queue_driver == 'database') {
                ...
                // Check valid path
                $check = Tool::checkPHPBinPath($php_bin_path_value);
                if ($check != 'ok') {
                ...
                Setting::set('php_bin_path', $php_bin_path_value);
            ...
            if (in_array($queue_driver, ['database', 'async']) && config('queue.default') != $queue_driver) {
                Setting::setEnv('QUEUE_DRIVER', $queue_driver);
            }
    ...
    public static function embedRemoteImages(string $content, AbsCampaign $campaign, ExtendedSwiftMessage $message)
    {
        ...
        foreach ($srcs as $src) {
            ...
            if (empty(trim($new_src))) {
                continue;
            }

            $path = $campaign->getStoragePath($new_src);
            if (\Illuminate\Support\Facades\File::exists($path)) {
                $entity = \Swift_EmbeddedFile::fromPath($path);
                $cid = $message->embed($entity);

                try {
                    $content = preg_replace('/(<img[^>]+src=)([\'"])(' . preg_quote($src, '/') . ')(\2)/', "$1$2$cid$4", $content);
    ...
    public static function replaceTemplateUrl($content, $host, $path)
    {
        ...
        foreach ($srcs as $src) {
            ...
            if (preg_match('/^cid:/i', $src) || preg_match('/https?/i', $src) || preg_match('/^\{/i', $src)
                || preg_match('/^\%/i', $src) || preg_match('/^https?/i', $src) || preg_match('/^#/i', $src)
                || strpos($src, '//') === 0 || preg_match('/^mailto:/i', $src) || preg_match('/^tel:/i', $src)) {
                // nothing
            } else if (strpos($src, '/') === 0) {
                $new_src = $host . $new_src;
            } else if (filter_var($src, FILTER_VALIDATE_EMAIL)) {
                $new_src = "mailto:" . $src;
            } else {
                // Build the full path
                $new_src = join_paths($host, $path, $new_src);
            }
    ...
    public static function currentTimezone()
    {
        if (self::currentView() == 'frontend') {
            $timezone = is_object(\Auth::user()) && is_object(request()->selected_customer) ? request()->selected_customer->timezone : '+00:00';

...
class EmailVerificationServerPolicy
{
    ...
    public function read(User $user, EmailVerificationServer $server, $role)
    {
        switch ($role) {
            case 'admin':
                ...
                break;
            case 'customer':
                $can = request()->selected_customer->getOption('create_email_verification_servers') == 'yes';
                break;
        }
    ...
    public function create(User $user, EmailVerificationServer $server, $role)
    {
        switch ($role) {
            case 'admin':
                ...
                break;
            case 'customer':
                $max = request()->selected_customer->getOption('email_verification_servers_max');
                $can = request()->selected_customer->getOption('create_email_verification_servers') == 'yes'
                    && (request()->selected_customer->emailVerificationServersCount() < $max || $max == -1);
                break;
        }
    ...
    public function update(User $user, EmailVerificationServer $server, $role)
    {
        switch ($role) {
            case 'admin':
                ...
                break;
            case 'customer':
                $can = request()->selected_customer->getOption('create_email_verification_servers') == 'yes' && request()->selected_customer->id == $server->customer_id;
                break;
        }
    ...
    public function delete(User $user, EmailVerificationServer $server, $role)
    {
        switch ($role) {
            ...
            case 'customer':
                $can = request()->selected_customer->getOption('create_email_verification_servers') == 'yes'
                    && request()->selected_customer->id == $server->customer_id;
                break;
        }

    ...
    public function import(User $user, Blacklist $blacklist, $role)
    {
        switch ($role) {
            case 'admin':
                ...
                break;
            case 'customer':
                $can = request()->selected_customer->getActiveImportBlacklistJobs()->count() == 0;
                break;
        }
    ...
    public function importCancel(User $user, Blacklist $blacklist, $role)
    {
        switch ($role) {
            case 'admin':
                ...
                break;
            case 'customer':
                $can = request()->selected_customer->getActiveImportBlacklistJobs()->count();
                break;
        }
    ...
    public function update(User $user, Blacklist $blacklist, $role)
    {
        switch ($role) {
            case 'admin':
                ...
                break;
            case 'customer':
                $can = request()->selected_customer->id == $blacklist->customer_id;
                break;
        }

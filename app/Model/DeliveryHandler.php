...
class DeliveryHandler extends Model
{
    ...
    public function test()
    {
        ...
        if ($inbox == false) {
            MailLog::info($this->encryption);
                ...
                $imapPath = "{{$this->host}:{$this->port}/{$this->protocol}/{$this->encryption}/novalidate-cert}INBOX";
                $inbox = @\imap_open($imapPath, $this->username, $this->password);
        }
    ...
    {
        preg_match('/(?<=X-Emailwish-Message-Id:)\s{0,1}<{0,1}(?<id>[a-zA-Z0-9\.]+[a-zA-Z0-9]+@[a-zA-Z0-9\.\-]+[a-zA-Z0-9]+)/', $message, $matched);

...
class OldSubscription extends Model
{
        ...
        if (isset($this->plan_id)) {
            ...
            switch ($plan->frequency_unit) {
                ...
                case 'year':
                    ...
                    break;
                case 'unlimited':
                    $this->start_at = Tool::dateTime(\Carbon\Carbon::now());
                    $this->end_at = null;
                    break;
            }
        }
    ...
    public function displayFileSizeUpload()
    {
        if ($this->getOption('max_file_size_upload') == -1) {
            ...
            return Tool::format_number($this->getOption('max_file_size_upload'));
        }
    }
    ...
    {
        $strs = [$this->plan_name];
        $strs[] = '('. Tool::formatDate($this->start_at);
        $strs[] = ' - '. Tool::formatDate($this->end_at).')';
        if ($this->isCurrent()) {

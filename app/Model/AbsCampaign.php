 ...
 * @property boolean sign_dkim
 * @property boolean track_open
 * @property boolean track_click
 * @property boolean resend
 * @property integer custom_order
 * @property Carbon|string run_at
 * @property Carbon|string delivery_at
 ...
 * @property integer default_mail_list_id
 * @property MailList defaultMailList
 * @property string cache
 * @property integer tracking_domain_id
 * @property TrackingDomain trackingDomain
 * @property boolean use_default_sending_server_from_email
 * @property string preheader
 *
 * @property MailList[] mailLists
 * @property CampaignsListsSegment[] listsSegments
 * @property Link[] links
 * @property TrackingLog[] trackingLogs
 * @property BounceLog[] bounceLogs
 * @property OpenLog[] openLogs
 * @property ClickLog[] clickLogs
 * @property FeedbackLog[] feedbackLogs
...
abstract class AbsCampaign extends Model
{
    ...
    const COLUMN_from_email = "from_email";
    ...
    const COLUMN_resend = "resend";
    const COLUMN_custom_order = "custom_order";
    const COLUMN_run_at = "run_at";
    const COLUMN_delivery_at = "delivery_at";
    const COLUMN_created_at = "created_at";
    const COLUMN_updated_at = "updated_at";
    const COLUMN_template_source = "template_source";
    const COLUMN_last_error = "last_error";
    const COLUMN_is_auto = "is_auto";
    const COLUMN_image = "image";
    const COLUMN_default_mail_list_id = "default_mail_list_id";
    const COLUMN_cache = "cache";
    const COLUMN_tracking_domain_id = "tracking_domain_id";
    const COLUMN_use_default_sending_server_from_email = "use_default_sending_server_from_email";
    const COLUMN_preheader = "preheader";
    const COLUMN_total_number_of_sales = "total_number_of_sales";
    const COLUMN_sales_total = "sales_total";
    ...
    {
        return $this->belongsTo(MailList::class, 'default_mail_list_id');
    }
    ...
    {
        return $this->belongsToMany(MailList::class, 'campaigns_lists_segments');
    }
    ...
    public function getListsSegmentsGroups()
    {
        ...
        foreach ($lists_segments as $lists_segment) {
            if (!isset($groups[$lists_segment->mail_list_id])) {
                ...
                $groups[$lists_segment->mail_list_id]['segment2_uids'] = [];
            }
            ...
            if (is_object($lists_segment->segment2) && !in_array($lists_segment->segment2->uid, $groups[$lists_segment->mail_list_id]['segment2_uids'])) {
                $groups[$lists_segment->mail_list_id]['segment2_uids'][] = $lists_segment->segment2->uid;
            }
        }
     ...
     *
     * @param $subscriber
     * @param SendingServer|null $server
     * @return array text message
     */
    public function prepareEmail($subscriber, $server = null)
    {
        MailLog::info("Preparing email `{$subscriber->email}`");
        $this->log('Preparing email',  $this->customer, ["Preparing email `{$subscriber->email}`"]);
        // build the message
        ...
        if ($this->type == self::TYPE_REGULAR) {
            $message->addPart($this->getHtmlContent($subscriber, $msgId, $server, $message), 'text/html');
        }

        // Always sign with DKIM
        MailLog::info("Adding DKIM signature");
        $this->log('Adding DKIM signature',  $this->customer, ["Adding DKIM signature"]);
        ...
        MailLog::info("Adding attachments `{$subscriber->email}`");
        $this->log('Adding attachments',  $this->customer, ["Adding attachments `{$subscriber->email}`"]);
        ...
        // Try to embed an image
        //$embed = \Swift_EmbeddedFile::fromPath("/home/volobot/Projects/Ankit/Email Wish/emailwish-v4/public/assets/images/login_cover.jpg");
        //$message->embed($embed);
    ...
    public function trackMessage($response, $subscriber, $server, $msgId)
    {
        ...
        $this->customer->countUsage($msgId);
        $server->countUsage();
    }
    ...
    public function getCustomHeaders($subscriber, $server)
    {
        ...
        return array(
            ...
            'X-Emailwish-Customer-Id' => $this->customer->uid,
     ...
     * @param $msgId
     * @param $server
     * @param ExtendedSwiftMessage|null $message
     * @return string
     */
    public function getHtmlContent($subscriber = null, $msgId = null, $server = null, $message = null)
    {
        $this->log('hmlt content',  $this->customer, ['started']);
        ...
        if (is_null($this->parsedContent)) {
            ...
            $id = $subscriber->id ?? "";
            $body = ChatSession::getHtmlRepresentation($body, $this->customer, $id ?? '', null);
            // STEP 05. Embed remote images
            //$body = Tool::embedRemoteImages($body, $this, $message);
        ...
        $this->log('hmlt content after parsedcontent',  $this->customer, [$body]);
...
'. $body .'
        ...
        $this->log('mediaquery added',  $this->customer, [$body]);
    ...
    {
        return $this->customer->getActiveShopifyRecurringApplicationCharge()->plan->getOption('email_footer_enabled') == 'yes';
    }
    ...
    {
        return $this->customer->getActiveShopifyRecurringApplicationCharge()->plan->getOption('html_footer');
    }
    ...
    {
        return $this->customer->getActiveShopifyRecurringApplicationCharge()->plan->getOption('plain_text_footer');
    }
    ...
    public function getPlainContent($subscriber, $msgId, $server = null)
    {
        ...
        $plain = $this->tagMessage($plain, $subscriber, $msgId, $server);

        // Replace bare line feed char which is not accepted by Outlook, Yahoo, AOL...
        $plain = StringHelper::replaceBareLineFeed($plain);
        $this->log('plain content',  $this->customer, [$plain]);
        return $plain;
    }
     ...
     *
     * @param ExtendedSwiftMessage $message
     * @param SendingServer|null $server
     * @return mixed
     */
    public function sign(ExtendedSwiftMessage $message, $server=null)
    {
        ...
        MailLog::info("Signing with domain DKIM");
    ...
    public function unsubscribe_url_valid()
    {
        if ($this->type != 'plain-text' &&
           request()->selected_customer->getOption('unsubscribe_url_required') == 'yes' &&
    ...
    public function step()
    {
        ...
        if (!empty($this->name) && !empty($this->subject) && !empty($this->from_name)
                && !empty($this->reply_to) && (!empty($this->from_email) || $this->use_default_sending_server_from_email)) {
            $step = 2;
        } else {
            return $step;
        }
    ...
    public function generateUnsubscribeUrl($msgId, $subscriber)
    {
        ...
        if (is_a($subscriber, 'stdClass')) {
            $path = route('unsubscribeUrl', ['message_id' => StringHelper::base64UrlEncode($msgId)], true);
    ...
    public function copy($name)
    {
        ...
        /** @var static $newCampaign */
        $newCampaign = Campaign::find($copy->id);
        $newCampaign->updateCache();

        // copy template
        Tool::xcopy($this->getStoragePath(), $newCampaign->getStoragePath());
     ...
     */
    public function resend($filter = 'not_receive') // not_receive | not_open | not_click | all
    {
    ...
    {
        $rules = [
            'lists_segments' => 'required|array|min:0',
        ];

        if (isset($params['lists_segments'])) {
            foreach ($params['lists_segments'] as $key => $param) {
                $rules['lists_segments.'.$key.'.mail_list_uid'] = 'required|string';
                $rules['lists_segments.'.$key.'.is_default'] = 'string';
                $rules['lists_segments.'.$key.'.segment_uids'] = 'array|min:0';
                $rules['lists_segments.'.$key.'.segment_uids.*'] = 'string';
                $rules['lists_segments.'.$key.'.segment2_uids'] = 'array|min:0';
                $rules['lists_segments.'.$key.'.segment2_uids.*'] = 'string';
            }
        }
    ...
    public function fillRecipients($params = [])
    {
        if (isset($params['lists_segments'])) {
            foreach ($params['lists_segments'] as $key => $param) {
                ...
                if (!empty($param['segment2_uids'])) {
                    foreach ($param['segment2_uids'] as $segment_uid) {
                        $segment = Segment2::findByUid($segment_uid);
                        if(!$segment) continue;
                        ...
                        $lists_segment->segment2_id = $segment->id;
                        $this->listsSegments->push($lists_segment);
                    }
                } else {
                    $lists_segment = new CampaignsListsSegment();
    ...
    public function saveRecipients($params = [])
    {
        ...
        if(empty($this->default_mail_list_id)){
            throw ValidationException::withMessages([
                'lists_segments' => "Please select a default mailing list"
            ]);
        }
        ...
        foreach ($lists_segments_groups as $lists_segments_group) {
            ...
            } else if (!empty($lists_segments_group['segment2_uids'])) {
                foreach ($lists_segments_group['segment2_uids'] as $segment_uid) {
                    $segment = Segment2::findByUid($segment_uid);
                    $data[] = [
                        'campaign_id' => $this->id,
                        'mail_list_id' => $lists_segments_group['list']->id,
                        'segment_id' => null,
                        'segment2_id' => $segment->id,
                    ];
                }
            } else {
                $data[] = [
    ...
    static function updateCaches()
    {
        $sending_campaigns = static::query()->where(static::COLUMN_status, static::STATUS_SENDING)->cursor();
        /** @var static $campaign */
    ...
    public function updateCache($key = null)
    {
        ...
        $index = [
            ...
            'SubscriberCount' => function (&$campaign) {
                return $campaign->subscribersCount(); // spepcial key that requires true update
            },
    ...
    public function openUniqHours($start = null, $end = null)
    {
        ...
        return $query->orderBy('open_logs.created_at', 'asc')->get()->groupBy(function ($date) {
            return Tool::dateTime($date->created_at)->format('H'); // grouping by hours
        });
    }
    ...
    public function transformWithoutUpdate($html, $useTrackingHost = false)
    {
        ...
        // Replace #1
        $path = route('campaign_assets', ['uid' => $this->uid, 'name' => ''], false);
    ...
    public function untransform()
    {
        ...
        $this->html = str_replace(
            route('campaign_assets', ['uid' => $this->uid, 'name' => '']).'/',
            '',
    ...
    public function copyFromTemplate($template)
    {
        ...
        $campaign_storage = route('campaign_assets', ['uid' => $this->uid, 'name'=>""], false);
        $this->html = str_replace($template_storage, $campaign_storage, $this->html);
        $this->html = $this->replaceInternalTags($this->html);

        $this->save();
    }

    public function replaceInternalTags($message)
    {
    ...
    public function makeSampleData($params = [])
    {
        ...
        $default = [
            'unconfirmed' => 0.08,
            'delivery_failed' => 0.05, // against all deivered emails
            'open' => 0.41, // against delivery_success
            'click' => 0.38, // against open
            'unsubscribe' => 0.08, // against open
            'bounce' => 0.04, // against open
            'feedback' => 0.02 // against open
        ];

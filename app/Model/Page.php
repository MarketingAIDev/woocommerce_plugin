...
class Page extends Model
{
    ...
    public function renderContent($values = null, $subscriber = null)
    {
        ...
        $this->content = str_replace('{CONTACT_COUNTRY}', $this->mailList->contact->countryName(), $this->content);
        ...
        $this->content = str_replace('{UNSUBSCRIBE_BUTTON}', '<button class="btn btn-info bg-black" type="submit">'.trans('messages.unsubscribe').' <i class="icon-arrow-right14 position-right"></i></button>', $this->content);
    ...
    public function getTransformedSubject($subscriber)
    {
        ...
        $subject = str_replace('{CONTACT_COUNTRY}', $this->mailList->contact->countryName(), $subject);

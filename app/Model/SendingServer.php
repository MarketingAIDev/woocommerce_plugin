 ...
 * @property integer bounce_handler_id
 * @property BounceHandler bounceHandler
 * @property integer feedback_loop_handler_id
 * @property FeedbackLoopHandler feedback_loop_handler
...
class SendingServer extends Model
{
    ...
    /** @var QuotaTrackerFile $quotaTracker */
    ...
    {
        return $this->hasMany(TrackingLog::class, 'sending_server_id')->orderBy('created_at', 'asc');
    }
    ...
    public function sendTestEmail($params)
    {
        ...
        $message->setId($msgId);
        $message->getHeaders()->addTextHeader('X-Emailwish-Message-Id', $msgId); // this header is required for SendGrid API sending server
     ...
     * @return mixed
     */
    public function sign(\Swift_Message $message)
    {
        MailLog::info("Signing with Sending server DKIM");
        if (strpos($this->host, 'amazonaws.com') !== false) {
            $message->setFrom($this->default_from_email);
        }
        $message->setSender($this->default_from_email);
        $privateKey = $this->dkim_private;
        $selector = $this->dkim_selector;
        $from = $this->default_from_email;
        if(empty($privateKey) || empty($selector) || empty($from)){
            MailLog::info("{$from} {$selector} {$privateKey}");
            MailLog::info("Sending server doesn't have DKIM credentials. Signing skipped");
            return $message;
        }
        $domainName = substr(strrchr($from, '@'), 1);
    ...
    {
        $domainName = substr(strrchr($this->default_from_email, '@'), 1);
        $dkim_host = $this->dkim_selector.'._domainkey.'.$domainName.'.';
        $dkim_value = sprintf('v=DKIM1; k=rsa; p=%s;', $this->getCleanPublicKey());
        return [$dkim_host, $dkim_value];
    }

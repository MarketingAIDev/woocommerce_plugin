 ...
 * @property string frequency_amount
 ...
 * @property boolean tax_billing_required
 * @property integer paddle_plan_id
...
class Plan extends Model
{
    ...
    public function validationRules()
    {
        $rules = [
            'general' => [
                ...
                'plan.general.usage_rate' => 'required|min:0',
    ...
    {
        $rules = array(
            'options.email_max_free' => 'required',
    ...
    {
        return []; // todo: replace with permanent fix
    ...
    public function displayQuota()
    {
        if ($this->getOption('sending_quota') == -1) {
            ...
            return strtolower(Tool::format_number($this->getOption('sending_quota')).' '.trans('messages.'. Tool::getPluralPrase('email', $this->getOption('sending_quota'))).' / '.$this->getOption('sending_quota_time').' '.trans('messages.'. Tool::getPluralPrase($this->getOption('sending_quota_time_unit'), $this->getOption('sending_quota'))));
        }
    }
    ...
    {
            return Tool::format_number($this->getOption('email_max') + $this->getOption('email_max_free'));
    }
     ...
     * @return string
     */
    public function displayFreeQuota()
    {
            return Tool::format_number($this->getOption('email_max_free'));
    }
    ...
    public function displayMaxCampaign()
    {
        if ($this->getOption('campaign_max') == -1) {
            ...
            return Tool::format_number($this->getOption('campaign_max'));
        }
    }
    ...
    public function displayMaxSizeUploadTotal()
    {
        if ($this->getOption('max_size_upload_total') == -1) {
            ...
            return Tool::format_number($this->getOption('max_size_upload_total'));
        }
    }
    ...
    public function getMaximumUsageChargeAttribute()
    {
        $usage_rate = $this->usage_rate ?? 0.00;
        $maximum_mails = $this->getOption('email_max') ?? 0.00;

        return $usage_rate * $maximum_mails / 1000;
    }
    /**
    ...
    public static function select2($request)
    {
        ...
        foreach ($query->limit(20)->get() as $plan) {
            $data['items'][] = ['id' => $plan->uid, 'text' => htmlspecialchars($plan->name).'|||'.htmlspecialchars(Tool::format_price($plan->price, $plan->currency->format))];
        }
    ...
    public function displayFrequencyTime()
    {
        ...
        return $this->frequency_amount.' '. Tool::getPluralPrase($this->frequency_unit, $this->frequency_amount);
    }
    ...
    {
        return 0; // todo: replace with permanent fix
    ...
    {
        return 0; // todo: replace with permanent fix
        //$this->subscriptions()->distinct('user_id')->count('user_id');
    }

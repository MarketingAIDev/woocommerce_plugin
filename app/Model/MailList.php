 ...
 * @property string email_subscribe
 * @property string email_unsubscribe
 * @property string email_daily
 * @property boolean send_welcome_email
 * @property boolean unsubscribe_notification
 ...
 * @property boolean subscribe_confirmation
 * @property string cache
 * @property bool all_sending_servers
 */
class MailList extends Model
{
    ...
    const COLUMN_default_subject = "default_subject";
    const COLUMN_from_email = "from_email";
    const COLUMN_from_name = "from_name";
    const COLUMN_remind_message = "remind_message";
    const COLUMN_email_subscribe = "email_subscribe";
    const COLUMN_email_unsubscribe = "email_unsubscribe";
    const COLUMN_email_daily = "email_daily";
    const COLUMN_send_welcome_email = "send_welcome_email";
    const COLUMN_unsubscribe_notification = "unsubscribe_notification";
    const COLUMN_shopify_shop_id = "shopify_shop_id";
    const COLUMN_status = "status";
    const COLUMN_created_at = "created_at";
    const COLUMN_updated_at = "updated_at";
    const COLUMN_custom_order = "custom_order";
    const COLUMN_subscribe_confirmation = "subscribe_confirmation";
    const COLUMN_cache = "cache";
    const COLUMN_all_sending_servers = "all_sending_servers";
    ...
    protected $fillable = [
        ...
        'subscribe_confirmation', 'all_sending_servers', 'shopify_shop_id'
    ];
    ...
    public function shopify_shop(): BelongsTo
    {
    ...
    {
        return $this->belongsToMany(Campaign::class, 'campaigns_lists_segments', 'mail_list_id', 'campaign_id');
    }
    ...
    {
        return request()->selected_customer->lists()->where('id', '!=', $this->id)->get();
    }
    ...
    public function send($message, $params = [])
    {
        ...
        $message->getHeaders()->addTextHeader('X-Emailwish-Message-Id', StringHelper::generateMessageId(StringHelper::getDomainFromEmail($this->from_email)));
    ...
    public function import2($file, $customer, $system_job)
    {
        try {
            ...
            each_batch($results, config('app.import_batch_size'), true, function ($batch) use ($logger, $availableFields, &$customer, &$processed_count, &$system_job, &$overQuotaAttempt) {
                ...
                // authorization
                if ($customer->user->can('addMoreSubscribers', [$this, sizeof($batch), $customer])) {
                ...
                $data = Arr::where($data, function ($record) use ($logger) {
    ...
    public function import3($file)
    {
        try {
            ...
            each_batch($results, config('app.import_batch_size'), true, function ($batch) use ($availableFields) {
                ...
                // authorization
                if (!$this->customer->user->can('addMoreSubscribers', [$this, config('app.import_batch_size'), $this->customer])) {
                    // If use cannot create ANY other subscribers
                    if (!$this->customer->user->can('addMoreSubscribers', [$this, 1, $this->customer])) {
                ...
                // validate amd remove invalid records
                $data = Arr::where($data, function ($record) {
                    list($valid, $errors) = $this->validateCsvRecord($record);
                    if (!$valid) {
    ...
    {
        return $this->hasMany(MailListsSendingServer::class);
    }
    ...
    public function getCurrentSubscription()
    {
        if (empty($this->currentSubscription)) {
            $this->currentSubscription = $this->customer->getActiveShopifyRecurringApplicationCharge();
        }
     ...
     *
     * @return SendingServer|null sending server
     */
    ...
    public function getSendingServers()
    {
        ...
        if ($this->customer->getOption('sending_server_option') == Plan::SENDING_SERVER_OPTION_OWN) {
        ...
        } elseif ($this->customer->getOption('sending_server_option') == Plan::SENDING_SERVER_OPTION_SYSTEM) {

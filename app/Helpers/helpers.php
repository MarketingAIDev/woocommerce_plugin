...
{
    $offset = Carbon::now($timezone)->offsetHours - Carbon::now('UTC')->offsetHours;
...
{
    ServerTiming::start('isInitiated');
    $initiated = file_exists(storage_path('app/installed'));
    ServerTiming::stop('isInitiated');
    return $initiated;
}
...
{
    return Tool::dateTime($datetime)->format('Y-m-d');
}
...
{
    return Tool::dateTime($datetime)->format('h:i A');
}
...
function react_route($path, $data = [])
{
    $react_app_url = env('REACT_APP_URL', 'https://dashboard.emailwish.com');
    foreach ($data as $k => $v) {
        str_replace("{$k}", $v, $path);
    }
    $path = implode('/', array_map('urlencode', explode('/', $path)));
    return $react_app_url . $path;
}
 ...
 */
function custom_validate($data, $rules, $error_prefix = '', $messages = [], $customAttributes = [])
{
    ...
    if ($validator->fails()) {
        ...
        foreach ($validation_errors as $key => $error) {
            $prefixed_errors[$error_prefix . $key] = $error;
        }
        throw ValidationException::withMessages($prefixed_errors);
    }

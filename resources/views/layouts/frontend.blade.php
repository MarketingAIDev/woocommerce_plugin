...
<body class="navbar-top color-scheme-{{ request()->selected_customer->getColorScheme() }}">
	...
	<!-- Main navbar -->
	<div class="navbar navbar-{{ request()->selected_customer->getColorScheme() == "white" ? "default" : "inverse" }} navbar-fixed-top">
		<div class="navbar-header">
			<a class="navbar-brand" href="{{ action('HomeController@index') }}">
                ...
                @else
                    <img src="{{ URL::asset('images/default_site_logo_small_' . (request()->selected_customer->getColorScheme() == "white" ? "dark" : "light") . '.png') }}" alt="">
                @endif
			</a>
		...
		<div class="navbar-collapse collapse" id="navbar-mobile">
			<ul class="nav navbar-nav">
				...
				@if (
					request()->selected_customer->can("read", new Acelle\Model\SendingServer()) ||
                    request()->selected_customer->can("read", new Acelle\Model\EmailVerificationServer()) ||
					request()->selected_customer->can("read", new Acelle\Model\Blacklist()) ||
					true
				)
			...
			<ul class="nav navbar-nav navbar-right">
				...
				<li class="dropdown dropdown-user">
					<a class="dropdown-toggle" data-toggle="dropdown">
						<img src="{{ action('CustomerController@avatar', request()->selected_customer->uid) }}" alt="">
						<span>{{ request()->selected_customer->displayName() }}</span>
						<i class="caret"></i>

						@if (request()->selected_customer->hasSubscriptionNotice())
					...
					<ul class="dropdown-menu dropdown-menu-right">
						@can("admin_access", Auth::user())
							...
							<li class="divider"></li>
						@endif
						@if (false && request()->user()->customer->activeSubscription())
							<li class="dropdown">

	...
	@if (null !== Session::get('orig_customer_id') && request()->selected_customer)
	...
	<div class="ui-pnotify alert-warning mc-notify-wanring" style="width: auto; right: 20px; top: auto; bottom: 20px; opacity: 1; display: block; overflow: visible; cursor: auto;">
		...
		@if (\Acelle\Library\Tool::currentView() == 'frontend')
			...
			@if (
				is_object(request()->selected_customer) &&
				request()->selected_customer->getOption("sending_server_option") == \Acelle\Model\Plan::SENDING_SERVER_OPTION_OWN &&
				!request()->selected_customer->activeSendingServers()->count())
				<div class="alert ui-pnotify-container alert-warning ui-pnotify-shadow" style="min-height: 16px; overflow: hidden;">
			...
			@if (is_object(request()->selected_customer) &&
				!is_object(request()->selected_customer->getActiveShopifyRecurringApplicationCharge()))
				<div class="alert ui-pnotify-container alert-warning ui-pnotify-shadow" style="min-height: 16px; overflow: hidden;">
		...
		<div class="modal-dialog modal-md">
			...
			<div class="modal-content">
				<form action="" method="POST" class="ajax_upload_form form-validate-jquery">
					...
					<div class="modal-footer">
						<button type="button" class="btn btn-default" data-dismiss="modal">{{ trans('messages.close') }}</button>
						<button type="submit" class="btn btn-info bg-teal">{{ trans('messages.submit') }}</button>
					</div>
				</form>
			</div>
		</div>

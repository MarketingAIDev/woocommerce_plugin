...
@if ($system_job->status == \Acelle\Model\SystemJob::STATUS_DONE)
    ...
    <br>
    <a href="{{ action('Admin\BlacklistController@import') }}" class="btn btn-info bg-teal mr-5">
        <i class="icon-reload-alt"></i> {{ trans('messages.blacklist.import_another') }}
    </a>
    <a href="{{ action('Admin\BlacklistController@index') }}" class="btn btn-info bg-grey">

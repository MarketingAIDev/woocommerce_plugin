...
@section('content')
	<form action="{{ action('InstallController@cronJobs') }}" method="POST" class="form-validate-jqueryz">
		...
		<div class="text-right">
			@if($valid)
				<a href="{{ action('InstallController@cronJobs') }}" class="btn btn-info bg-grey">
					<i class="icon-gear"></i> {!! trans('messages.change_cronjob_setting') !!}
				</a>
				<a href="{{ action('InstallController@finishing') }}" class="btn btn-info bg-teal">
					{!! trans('messages.next') !!} <i class="icon-arrow-right14 position-right"></i>
				</a>				
			@else
				<button type="submit" class="btn btn-info bg-teal">

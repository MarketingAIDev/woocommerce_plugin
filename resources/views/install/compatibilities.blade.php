...
@section('content')
	...
	<div class="text-right">                                    
		@if ($result)
			<a href="{{ action('InstallController@siteInfo') }}" class="btn btn-info bg-teal">{!! trans('messages.next') !!} <i class="icon-arrow-right14 position-right"></i></a>
		@else
			<a href="{{ action('InstallController@systemCompatibility') }}" class="btn btn-info bg-grey-600"><i class="icon-reload-alt position-right"></i> {!! trans('messages.try_again') !!}</a>
		@endif
	</div>

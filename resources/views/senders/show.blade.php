        ...
        <div class="col-sm-12 col-md-8">
            ...
            <form enctype="multipart/form-data" action="{{ action('SenderController@verify', $sender->uid) }}" method="POST" class="form-vsalidate-jquery">
                ...
                @if ($sender->status == Acelle\Model\Sender::STATUS_NEW)
                    <button class="btn btn-info">{{ trans('messages.sending_domain.verify') }}</button>
                @endif
                <a href="{{ action('SenderController@edit', $sender->uid) }}" class="btn btn-info bg-grey" style="min-width: 100px"><i class="icon-pencil"></i> {{ trans('messages.sender.edit') }}</a>
            </form>
        </div>
    </div>
@endsection
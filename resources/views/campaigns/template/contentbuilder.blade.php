<?php
/** @var Acelle\Model\Campaign $campaign */
?>
<!DOCTYPE HTML>
<html>
<head>
...
<script>
    ...
    let builder = new ContentBuilder({
        ...
        toolbarDisplay: 'auto',
        toolStyle: '',
        uploadAssetUrl: '<?= action([\Acelle\Http\Controllers\CampaignController::class, 'templateAsset'], ['uid'=>$campaign->uid])?>',
        ewTagsSource: '<?= action([\Acelle\Http\Controllers\CampaignController::class, 'tags'], ['uid'=>$campaign->uid])?>',
        ...
        storeDomain: '<?= $campaign->customer ? $campaign->customer->shopify_shop->primary_domain : ''?>',
        storeTheme: '<?= json_encode($campaign->customer ? $campaign->customer->shopify_shop->theme : []) ?>',
        storeShopCurrency: '<?= $campaign->customer ? $campaign->customer->shopify_shop->primary_currency: 'USD' ?>',
    ...
    function saveScreenshot() {
        const screenshotSaveURL = '{{ action([\Acelle\Http\Controllers\CampaignController::class, 'saveImage'], ['uid' =>$campaign->uid]) }}';
    ...
    btnGetHtml.addEventListener('click', function(){
        ...
        const saveURL = '{{ action([\Acelle\Http\Controllers\CampaignController::class, 'templateEdit'], ['uid' =>$campaign->uid]) }}';
        const csrf_token = "{{ csrf_token() }}";

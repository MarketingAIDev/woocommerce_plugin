...
@section('content')
    ...
    <form id="emailSetup" action="{{ action('Automation2Controller@emailSetup', $automation->uid) }}" method="POST">
        ...
        <div class="row">
            <div class="col-md-6">                                            
                ...
                @include('helpers.form_control', [
                    ...
                    'value' => $email->from_name ?? $automation->customer->displayName(),
                    'rules' => $email->rules(),
                    'help_class' => 'email',
                ...
                @include('helpers.form_control', [
                    ...
                    'value' => $email->from ?? $automation->customer->user->email,
                    'rules' => $email->rules(),
                    'help_class' => 'email',
                    'url' => action('SenderController@dropbox'),
                    'empty' => trans('messages.sender.dropbox.empty'),
                    'error' => trans('messages.sender.dropbox.error.' . request()->selected_customer->allowUnverifiedFromEmailAddress(), [
                ...
                @include('helpers.form_control', [
                    ...
                    'value' => $email->reply_to ?? $automation->customer->user->email,
                    'url' => action('SenderController@dropbox'),
                    'rules' => $email->rules(),
                    'help_class' => 'email',
                    'empty' => trans('messages.sender.dropbox.empty'),
                    'error' => trans('messages.sender.dropbox.reply.error.' . request()->selected_customer->allowUnverifiedFromEmailAddress(), [
                        'sender_link' => action('SenderController@index'),
                    ]),
                    'header' => trans('messages.verified_senders'),
                    'placeholder' => trans('messages.automation.email.from.placeholder'), 
            ...
            <div class="col-md-6 segments-select-box">
                <div class="form-group checkbox-right-switch">
                    ...
                    @include('helpers.form_control', [
                        ...
                        'value' => false, //$email->tracking_domain_id || request()->custom_tracking_domain,
                        'options' => [false,true],
                        'help_class' => '', //'email',
                        'rules' => $email->rules()
                    ])
                    ...
                    <div class="select-tracking-domain">
                        @include('helpers.form_control', [
                            ...
                            'options' => request()->selected_customer->getVerifiedTrackingDomainOptions(),
                ...
                <div class="automation2-tags">
                    <h6>Available Tags for Email Subject:</h6>
                    <div style="max-height:400px;overflow-y:auto;">
                    @foreach($tags as $tag)
                        <div class="btn btn-outline-secondary btn-sm d-block m-1" onclick="handleTagClick('{{$tag['tag']}}')">{{ $tag['name'] }}</div>
                    @endforeach
                    </div>
                </div>
            </div>
        ...
        <div class="text-right mt-5 {{ request()->selected_customer->allowUnverifiedFromEmailAddress() ? '' : 'unverified_next_but' }}">

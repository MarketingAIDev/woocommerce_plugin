...
@section('content')
    ...
    <header class="m-4">
        <nav class="bg-white box-shadow sticky-top navbar navbar-expand-lg navbar-light bg-light">
            <a class="navbar-brand" href="#">
                    <img src="{{ URL::asset('images/logo_emailwish.png?v=3') }}" alt="" style="width: 200px">
            </a>
            <div class="d-inline-block d-flex mr-auto align-items-center ">
                <a href="{{ react_route('/email/automations') }}" class="action heading-color heading-color-hover">
                    <i class="material-icons-outlined mr-2">arrow_back</i>
                    {{ trans('messages.automation.go_back') }}
                </a>
                &nbsp;&nbsp;&nbsp;
                <h1 class="heading-color">{{ $automation->name }}</h1>
                <i class="material-icons-outlined automation-head-icon ml-2">alarm</i>
            </div>
            <div class="automation-top-menu">
                <span class="mr-3">
                    <i class="last_save_time heading-color"
                       data-url="{{ action('Automation2Controller@lastSaved', $automation->uid) }}">
                        {{ trans('messages.automation.designer.last_saved', ['time' => $automation->updated_at->diffForHumans()]) }}
                    </i>
                </span>
                <div class="switch-automation d-flex">
                    <select class="select select2 top-menu-select heading-color heading-color-hover"
                            name="switch_automation">
                    ...
                    <a href="javascript:'" class="action heading-color heading-color-hover">
    ...
    <main role="main ">
        <div class="automation2 mx-4 box-shadow">
            <div class="diagram text-center scrollbar-inner" style="margin: 0;">
                <svg id="svg" style="overflow: auto" width="2800" height="1800" xmlns="http://www.w3.org/2000/svg"
                     xmlns:xlink="http://www.w3.org/1999/xlink">
                    <text x="475" y="30" alignment-baseline="middle"
                          text-anchor="middle">{{ trans('messages.automation.designer.intro') }}</text>
            ...
            <div class="sidebar scrollbar-inner" style="margin: 0;">
            ...
            tree = AutomationElement.fromJson(json, container, {
                ...
                validate: function (e) {
                    ...
                    if (e.getType() == 'ElementCondition') {
                        if (
                            e.getOptions()['type'] == null
                            || (e.getOptions()['type'] === '<?= Evaluate::CONDITION_open ?>' && e.getOptions()['email'] == null)
                            || (e.getOptions()['type'] === '<?= Evaluate::CONDITION_click ?>' && e.getOptions()['email_link'] == null)
                            || (e.getOptions()['type'] === '<?= Evaluate::CONDITION_review_image ?>' && false) // No extra validation required
                            || (e.getOptions()['type'] === '<?= Evaluate::CONDITION_review_stars ?>' && (e.getOptions()['review_stars_gte'] == null || e.getOptions()['review_stars_lte'] == null))
                            || (e.getOptions()['type'] === '<?= Evaluate::CONDITION_review_submitted ?>' && e.getOptions()['review_products_type'] == null)
                            || (e.getOptions()['type'] === '<?= Evaluate::CONDITION_chat_stars ?>' && (e.getOptions()['chat_stars_gte'] == null || e.getOptions()['chat_stars_lte'] == null))
                            || (e.getOptions()['type'] === '<?= Evaluate::CONDITION_discount_coupon_used ?>' && e.getOptions()['discount_coupon'] == null)
                            || (e.getOptions()['type'] === '<?= Evaluate::CONDITION_checkout_abandoned ?>' && false) // No extra validation required
                        ) {

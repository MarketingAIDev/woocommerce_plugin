    ...
    <div class="row mb-3">
        <div class="col-md-8">
            ...
            @include('helpers.form_control', [
                ...
                'type' => 'select',
                'label' => trans('messages.automation.choose_a_mail_list'),
                'value' => (is_object($automation->mailList) ? $automation->mailList->uid : ''),
                'options' => request()->selected_customer->getMailListSelectOptions(),
                'rules' => $automation->rules(),
                'disabled' => true,
            ])
            ...
            @include('helpers.form_control', [
                'name' => 'segment2_id',
                'include_blank' => trans('messages.automation.segment.all_list'),
                'type' => 'select',
                'label' => trans('messages.automation.segment'),
                'value' => $automation->segment2_id,
                'options' => request()->selected_customer->getSegment2SelectOptions(),
                'rules' => $automation->rules(),
                'disabled' => true,
            ])
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        addMaskLoading(
                            ...
                            function() {
                                window.location = '{{ react_route('/email/automations') }}';
                            },
                            { wait: 2000 }
                        );

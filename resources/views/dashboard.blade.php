...
@section('content')
    <h1 class="mb-10">{!! trans('messages.frontend_dashboard_hello', ['name' => request()->selected_customer->displayName()]) !!}</h1>
    ...
    <div class="row quota_box">
        <div class="col-sm-6 col-md-6">
            <div class="content-group-sm">
                ...
                <div class="progress progress-sm">
                    <div class="progress-bar bg-{{ request()->selected_customer->getSendingQuotaUsagePercentage() >= 80 ? 'danger' : 'primary' }}" style="width: {{ request()->selected_customer->getSendingQuotaUsagePercentage() }}%">
                    </div>
                </div>
            </div>
        </div>
        <div class="col-sm-6 col-md-6">
            <div class="content-group-sm">
                ...
                <div class="progress progress-sm">
                    <div class="progress-bar bg-{{ request()->selected_customer->listsUsage() >= 80 ? 'danger' : 'primary' }}" style="width: {{ request()->selected_customer->listsUsage() }}%">
                    </div>
                </div>
            </div>
        </div>
        <div class="col-sm-6 col-md-6">
            <div class="content-group-sm">
                <div class="pull-right  text-semibold">
                    <span class="text-muted">{{ \Acelle\Library\Tool::format_number(request()->selected_customer->campaignsCount()) }}/{{ \Acelle\Library\Tool::format_number(request()->selected_customer->maxCampaigns()) }}</span>
                    &nbsp;&nbsp;&nbsp;{{ request()->selected_customer->displayCampaignsUsage() }}
                </div>
                <h5 class="text-semibold mb-5 mt-20">{{ trans('messages.campaign') }}</h5>
                <div class="progress progress-sm">
                    <div class="progress-bar bg-{{ request()->selected_customer->campaignsUsage() >= 80 ? 'danger' : 'primary' }}" style="width: {{ request()->selected_customer->campaignsUsage() }}%">
                    </div>
                </div>
            </div>
        </div>
        <div class="col-sm-6 col-md-6">
            <div class="content-group-sm">
                <div class="pull-right  text-semibold">
                    <span class="text-muted">{{ \Acelle\Library\Tool::format_number(request()->selected_customer->readCache('SubscriberCount', 0)) }}/{{ \Acelle\Library\Tool::format_number(request()->selected_customer->maxSubscribers()) }}</span>
                    &nbsp;&nbsp;&nbsp;{{ request()->selected_customer->displaySubscribersUsage() }}
                </div>
                <h5 class="text-semibold mb-5 mt-20">{{ trans('messages.subscriber') }}</h5>
                <div class="progress progress-sm">
                    <div class="progress-bar bg-{{ request()->selected_customer->subscribersUsage() >= 80 ? 'danger' : 'primary' }}" style="width: {{ request()->selected_customer->readCache('SubscriberUsage', 0) }}%">
                    </div>
                </div>
            </div>
        </div>
    ...
    @if (request()->selected_customer->sentCampaigns()->count() == 0)
        ...
        <div class="row">
            <div class="col-md-6">
                @include('helpers.form_control', [
                    ...
                    'label' => '',
                    'value' => '',
                    'options' => Acelle\Model\Campaign::getSelectOptions(request()->selected_customer, "done"),
                ])
            </div>
        </div>
    ...
    @if (request()->selected_customer->lists()->count() == 0)
        <div class="empty-list">
        ...
        <div class="row">
            <div class="col-md-6">
                @include('helpers.form_control', [
                    ...
                    'options' => request()->selected_customer->readCache('MailListSelectOptions', []),
                ])
            </div>
        </div>
        ...
        <div class="tab-content">
            <div class="tab-pane active" id="top-tab1">
                <ul class="modern-listing mt-0 top-border-none">
                    @forelse (Acelle\Model\Campaign::topOpens(5, request()->selected_customer)->get() as $num => $item)
                        <li>
                            <div class="row">
            ...
            <div class="tab-pane" id="top-tab2">
                <ul class="modern-listing mt-0 top-border-none">
                    @forelse (Acelle\Model\Campaign::topClicks(5, request()->selected_customer)->get() as $num => $item)
                        <li>
                            <div class="row">
                                <div class="col-sm-5 col-md-5">
                                    <i class="number">{{ $num+1 }}</i>
            ...
            <div class="tab-pane" id="top-tab3">
                <ul class="modern-listing mt-0 top-border-none">
                    @forelse (Acelle\Model\Campaign::topLinks(5, request()->selected_customer)->get() as $num => $item)
                        <li>
                            <div class="row">
                                <div class="col-sm-6 col-md-6">
                                    <i class="number">{{ $num+1 }}</i>
                                ...
                                <div class="col-sm-2 col-md-2 text-left">
                                    <h5 class="no-margin text-bold">
                                        {{ (null !== $item->lastClick(request()->selected_customer)) ? Acelle\Library\Tool::formatDateTime($item->lastClick(request()->selected_customer)->created_at) : "" }}
                                    </h5>
                                    <span class="text-muted">{{ trans('messages.last_clicked') }}</span>
                                    <br /><br />
                                </div>
    ...
    @if (request()->selected_customer->logs()->count() == 0)
        <div class="empty-list">
        ...
        <div class="scrollbar-box action-log-box">
            ...
            <div class="timeline timeline-left content-group">
                <div class="timeline-container">
                        @foreach (request()->selected_customer->logs()->take(20)->get() as $log)
                            <!-- Sales stats -->

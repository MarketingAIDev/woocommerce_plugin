<?php
/** @var Acelle\Model\Template $template */
?>
<!DOCTYPE HTML>
<html lang="en">
<head>
...
<script src="/js/dom-to-image.min.js" type="text/javascript"></script>
<script>
    let win = null;
    let builder = new ContentBuilder({
        ...
        toolbarDisplay: 'auto',
        toolStyle: '',
        uploadAssetUrl: '<?= action([\Acelle\Http\Controllers\TemplateController::class, 'builderAsset'], ['uid'=>$template->uid])?>',
        ewTagsSource: '<?= action([\Acelle\Http\Controllers\TemplateController::class, 'tags'], ['uid'=>$template->uid])?>',
        ...
        storeDomain: '<?= $template->customer ? $template->customer->shopify_shop->primary_domain : ''?>',
        storeTheme: '<?= json_encode($template->customer ? $template->customer->shopify_shop->theme : []) ?>',
        storeShopCurrency: '<?= $template->customer ? $template->customer->shopify_shop->primary_currency: 'USD' ?>',
    ...
    function getHtml() {
        //const bgColor = bgColorSelector.value;
        bgColor = document.querySelector('.container_bg').style.backgroundColor
        return '<div class="container_bg" style="background-color:'+bgColor+'"><div class="container">'+builder.html()+'</div></div>';
    }

    function saveScreenshot() {
        const screenshotSaveURL = '{{ action([\Acelle\Http\Controllers\TemplateController::class, 'saveImage'], ['uid' =>$template->uid]) }}';
        const csrf_token = "{{ csrf_token() }}";
        const container = document.getElementsByClassName("container_bg")[0];
        domtoimage.toPng(container)
            .then(function (dataUrl) {
                console.log(dataUrl)
                $.post(screenshotSaveURL,
                    {
                        _token: csrf_token,
                        data: dataUrl
                    },
    ...
    btnGetHtml.addEventListener('click', function(){
        ...
        const saveURL = '{{ action([\Acelle\Http\Controllers\TemplateController::class, 'contentBuilderSave'], ['uid' =>$template->uid]) }}';
        const csrf_token = "{{ csrf_token() }}";
    ...
    //     if(!document.getElementById('container_bg_inner')){
    //         console.log('notexist')
    //         org_html = document.getElementsByClassName("is-builder")[0].innerHTML;
    //         new_html = "<div id='container_bg_inner'>" + org_html + "</div>";
    //         document.getElementsByClassName("is-builder")[0].innerHTML = new_html;
    //     }
    //     })
</script>
